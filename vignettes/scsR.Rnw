...
As an example, we use the analyzed data on the cell number of two genome-wide siRNA screens, performed with different siRNA libraries: QIAGEN unpooled and DHARMACON pooled. 
In both the screens Hela Cell Lines have been infected by a pathogen and the data has been collected and analyzed using latest technologies.
...
We added to this package the data for the cell number phenotype (that we are going to use in the following examples ). However we encrypted both the siRNA sequences and the gene identifiers for copyright issues, in a way that preserves the relation seed vs sequences (i.e. the groups of sequences that share the same seed remain the same) \\
